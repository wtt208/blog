...
```
如果Io_size>0x1000 读取0x1000字节到v6(栈上)，如果0x400<Io_size<0x1000，读取0x400字节到v6，如果Io_size<0x400,读取io_size字节到v6,v6_offset = 0x438 ,所以想覆盖ret必须要`io_size>0x1000
...
```c
v18 = __isoc23_strtoul(&v69[8], 0LL, 10LL);// SETBODY command - parse size parameter
          v19 = 1024LL;
          if ( v18 <= 0x400 )
            v19 = v18;
          v20 = v19;
...
A:因为 robust 机制只会“接管”它认为是 futex 锁的值，而 futex 锁的“合法形态”是
低 30 位 = 某个线程的 TID
...
### 1.set_robust_list(struct robust_list_head \*head, size_t len)
把当前线程的“robust futex 链表头”地址登记到内核
...
|字段|内核用来干嘛|
|---|---|
|`list.next`|所有当前线程持有的 robust 锁链表|
|`futex_offset`|从链表节点找到 futex 变量的位置|
|`list_op_pending`|正在“插入/删除”中的锁（防竞态）|

`size_t len`
必须等于sizeof(struct robust_list_head),64 位上是 **24 字节**
...
只要这个位被置位，就说明`原持锁线程已经在未解锁情况下退出`

这样别的线程就能检测到：
`pthread_mutex_lock → 返回 EOWNERDEAD`
...
if (r == EOWNERDEAD) {
    ...
    pthread_mutex_consistent(&mtx);    // 告诉系统“我修好了”
}
...
1. 取出你注册的 robust list
    
2. 遍历每个节点
    
3. 找到每个 futex 地址
    
4. 把这个地址上的值改成：tid | FUTEX_OWNER_DIED   (0x40000000)
...
线程退出时自动清理互斥锁相关的 futex
body_size= tid 
将body_size作为futex，但缺少内部状态位
\*uaddr = FUTEX_OWNER_DIED

正常流程应该是：
\*uaddr = FUTEX_OWNER_DIED | (uaddr 低30位原值)
new_value = 0x40000000 | (old_value & 0x3fffffff)
低 30 位不会变，只是强行 OR 上 0x40000000
